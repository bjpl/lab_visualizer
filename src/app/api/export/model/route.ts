...
function generateOBJ(
  ...
  for (let i = 0; i < data.vertices.length; i += 3) {
    const x = (data.vertices[i] ?? 0) * scale;
    const y = (data.vertices[i + 1] ?? 0) * scale;
    const z = (data.vertices[i + 2] ?? 0) * scale;
    obj += `v ${x} ${y} ${z}\n`;
  }
  ...
  if (options.includeNormals && data.normals.length > 0) {
    ...
    for (let i = 0; i < data.normals.length; i += 3) {
      obj += `vn ${data.normals[i] ?? 0} ${data.normals[i + 1] ?? 0} ${data.normals[i + 2] ?? 0}\n`;
    }
  }

  obj += '# Faces\n';
  for (let i = 0; i < data.indices.length; i += 3) {
    const v1 = (data.indices[i] ?? 0) + 1;
    const v2 = (data.indices[i + 1] ?? 0) + 1;
    const v3 = (data.indices[i + 2] ?? 0) + 1;
...
function generateSTL(
  ...
  for (let i = 0; i < data.indices.length; i += 3) {
    const i1 = (data.indices[i] ?? 0) * 3;
    const i2 = (data.indices[i + 1] ?? 0) * 3;
    const i3 = (data.indices[i + 2] ?? 0) * 3;

    const v1 = [
      (data.vertices[i1] ?? 0) * scale,
      (data.vertices[i1 + 1] ?? 0) * scale,
      (data.vertices[i1 + 2] ?? 0) * scale
    ];
    const v2 = [
      (data.vertices[i2] ?? 0) * scale,
      (data.vertices[i2 + 1] ?? 0) * scale,
      (data.vertices[i2 + 2] ?? 0) * scale
    ];
    const v3 = [
      (data.vertices[i3] ?? 0) * scale,
      (data.vertices[i3 + 1] ?? 0) * scale,
      (data.vertices[i3 + 2] ?? 0) * scale
    ];
...
function calculateNormal(v1: number[], v2: number[], v3: number[]): number[] {
  const u = [(v2[0] ?? 0) - (v1[0] ?? 0), (v2[1] ?? 0) - (v1[1] ?? 0), (v2[2] ?? 0) - (v1[2] ?? 0)];
  const v = [(v3[0] ?? 0) - (v1[0] ?? 0), (v3[1] ?? 0) - (v1[1] ?? 0), (v3[2] ?? 0) - (v1[2] ?? 0)];
  ...
  const length = Math.sqrt((normal[0] ?? 0) ** 2 + (normal[1] ?? 0) ** 2 + (normal[2] ?? 0) ** 2);
  if (length === 0) return [0, 0, 1];
  return [(normal[0] ?? 0) / length, (normal[1] ?? 0) / length, (normal[2] ?? 0) / length];
}
